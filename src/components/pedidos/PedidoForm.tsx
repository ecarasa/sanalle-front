...
export const parsePackQuantity = (producto: Producto): number => {
  return producto.blisters_por_caja || 1
}
...
export default function PedidoForm({
  ...
  // Fetch products when searching in modal or loading more pages
  useEffect(() => {
    if (!modalOpen) return
    const isFirstPage = productPage === 1
    if (isFirstPage) setLoadingProductos(true)
    else setLoadingMoreProducts(true)

    const fetchProductos = async () => {
      try {
        const res = await api.get('/productos', {
          params: { search: debouncedProductSearch, page: productPage, page_size: 20 },
        })
        const newItems = res.data.items
        setProductos((prev) => isFirstPage ? newItems : [...prev, ...newItems])
        setHasMoreProducts(newItems.length === 20)
      } catch {
    ...
    fetchProductos()
  }, [debouncedProductSearch, modalOpen, productPage])
  ...
  const getAvailableStock = useCallback((producto: Producto) => {
  ...
  const handleCantidadChange = (e: React.ChangeEvent<HTMLInputElement>, selectedProduct: Producto) => {
    ...
    if (cantidad > getAvailableStock(selectedProduct)) {
      toast.error(`No puedes pedir más de ${getAvailableStock(selectedProduct)} unidades`)
      return
    }
  ...
  useEffect(() => {
    ...
    const nextItems = itemsRef.current.map((item) => {
      const base = precioDeLista(item.producto_precios, tipoPrecio)
      if (base === null) {
  ...
  const handleProductListScroll = useCallback((e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (nearBottom && hasMoreProducts && !loadingProductos && !loadingMoreProducts) {
      setProductPage((prev) => prev + 1)
    }
  }, [hasMoreProducts, loadingProductos, loadingMoreProducts])
  ...
  const selectProduct = (producto: Producto) => {
    ...
    const base = precioDeLista(precios, tipoPrecio)
    const precioInicial = base ?? (esComercio ? 0 : producto.pvp ?? 0)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Order Info Card */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Datos del Pedido</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              value={numeroPedido || ''}
              readOnly
              className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-gray-50 text-gray-600 font-medium"
            />
          </div>
          ...
          <div>
            ...
            >
              ...
              <DateInput className="flex gap-0.5 w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-[#003087]/20 focus-within:border-[#003087] bg-white">
                {(segment) => (
                  <DateSegmentInput
                    segment={segment}
                    className="rounded px-0.5 outline-none focus:bg-[#003087] focus:text-white data-[placeholder]:text-gray-400"
                  />
                )}
              </DateInput>
            </DatePicker>
          ...
          <div>
            ...
            <input
              ...
              placeholder="OCA, Reparto propio..."
          ...
          <div>
            <DatePicker
              ...
              onChange={setFechaCompromisoPago}
            >
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Compromiso de Pago
              </label>
          ...
          <div className="flex items-center gap-3 pt-5">
            <input
              ...
              type="checkbox"
              checked={despachado}
              onChange={(e) => setDespachado(e.target.checked)}
              className="w-4 h-4 text-[#003087] border-gray-300 rounded focus:ring-[#003087]/20"
            />
            <label htmlFor="despachado" className="text-sm font-medium text-gray-700">
              Despachado
            </label>
          </div>
          ...
          <div>
            ...
            <input
              ...
              min={0}
              value={bultos}
              onChange={(e) => setBultos(parseInt(e.target.value) || 0)}
              className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#003087]/20 focus:border-[#003087]"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={formatCurrency(importeTotal)}
              readOnly
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-[#003087] rounded-lg hover:bg-[#002570] transition-colors"
          >
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-2 text-gray-700 text-right">{item.cantidad}</td>
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-medium text-white bg-[#003087] rounded-lg hover:bg-[#002570] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
            ...
            <div className="p-4 overflow-y-auto flex-1">
              {!selectedProduct ? (
                <>
                  ...
                  {/* Product list */}
                  <div className="space-y-1 max-h-[300px] overflow-y-auto" onScroll={handleProductListScroll}>
                    {loadingProductos ? (
